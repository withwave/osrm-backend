...
test('trip: trip in Monaco', function(assert) {
    ...
    osrm.trip({coordinates: [three_test_coordinates[0], three_test_coordinates[1]]}, function(err, trip) {
        assert.ifError(err);
...
test('trip: throws with too few or invalid args', function(assert) {
    assert.plan(2);
    var osrm = new OSRM(data_path);
    assert.throws(function() { osrm.trip({coordinates: [three_test_coordinates[0], three_test_coordinates[1]]}) },
